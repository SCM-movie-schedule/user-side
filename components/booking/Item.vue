...
<template>
    <div class=" mb-6 mr-3" >
        <NuxtLink  class=" block" :to="`/movies/${booking.movie.id}`">
            <img class=" block h-80 w-80"  :src="booking.movie.image.url" :alt="booking.movie.title">
        </NuxtLink>
        <div class=" flex flex-nowrap justify-between p-2 bg-gray-dark text-white">
            <h1 class=" font-bold">{{ booking.movie.title }}</h1>
            <div class="relative">
                <button @click="unBookMovie(booking.id)" class="  text-yellow-bright font-bold">Unbook</button>
