...
// booking
const {mutate, onDone, loading, onError } = mutation(bookMovie);
...
// rating
const {mutate:ratingMutate, onDone:ratingDone, loading:ratingLoading, onError:ratingOnError } = mutation(rateMovie);
const rate = (rating) => {
    if(authStore.isLoggedIn){
        const variables = {   
        data:{
                    ...
                    <ul class=" my-6 flex flex-wrap items-stretch ">
                        ...
                        <p class=" text-white mx-2 py-1">
                            <span class=" hover:text-yellow-bright cursor-pointer"  v-for="genere in props.movie.movies_generes" 
                                ...
                                @click="filterMoviesByGenere(genere.genere.id)"
                                >{{ genere.genere.name }},
                            </span>
                        </p>
